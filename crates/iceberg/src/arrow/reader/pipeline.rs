...
use super::{
    ...
    apply_name_mapping_to_arrow_schema, find_leaf_by_field_id, process_record_batch_stream,
};
...
use crate::metadata_columns::{
    ...
    RESERVED_FIELD_ID_SPEC_ID, is_metadata_field, row_pos_field,
};
...
//
// `pub(crate)` so `arrow::incremental` can apply the same metadata-only-projection
// row-count optimization (see `FileScanTaskReader::process`).
pub(crate) const PRUNABLE_METADATA_FIELDS: &[i32] = &[
...
impl FileScanTaskReader {
    async fn process(self, task: FileScanTask) -> Result<ArrowRecordBatchStream> {
        ...
        // positional fallback `_row_id` synthesis needs (`first_row_id + pos`), so request
        // it whenever `_row_id` is synthesized -- not just when `_pos` is itself projected.
        // A null `first_row_id` nulls the whole `_row_id` column instead (see below), so
        // nothing is synthesized and the virtual column isn't needed in that case.
        let need_row_number = project_row_id && task.first_row_id().is_some();

        // A projection consisting only of prunable metadata fields (e.g. `_file`,
        // `_spec_id`) has no data-column row-count source once the projection mask is
        // downgraded to read zero data columns; request the RowNumber virtual column so
        // the row count still survives in that case (see the mask-downgrade logic in
        // `build_projected_record_batch_stream`).
        ...
        let mut virtual_columns = ArrowReader::build_virtual_columns(task.project_field_ids());
        ...
        let open_fut = ArrowReader::open_parquet_stream_builder(
            ...
            virtual_columns,
            self.batch_size,
            task.name_mapping().map(|nm| nm.as_ref()),
        ...
        ArrowReader::build_projected_record_batch_stream(
            ...
            use_position_fallback,
            task.data_file_path(),
            task.partition_spec().cloned(),
            task.partition().cloned(),
            task.first_row_id(),
            task.data_sequence_number(),
            task.unified_partition_type().cloned(),
        )
    }
}
...
impl ArrowReader {
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) async fn apply_parquet_filters(
        ...
        if let Some(predicate) = bound_predicate {
            ...
            if bloom_filter_enabled {
                ...
                let candidate_rgs = match &selected_row_group_indices {
                    ...
                    None => {
                        all_rgs = (0..builder.metadata().num_row_groups()).collect::<Vec<_>>();
                        &all_rgs
                    }
                };
    ...
    pub(crate) fn build_projected_record_batch_stream(
        ...
        let project_row_id = project_field_ids.contains(&RESERVED_FIELD_ID_ROW_ID);

        let phys_row_id_leaf = if project_row_id {
            find_leaf_by_field_id(builder.parquet_schema(), RESERVED_FIELD_ID_ROW_ID)
        } else {
            None
        };
        ...
        // Gated on `!has_missing_field_ids` (i.e. not position fallback): under positional
        // fallback every column lacks an embedded id and synthetic ids are assigned by
        // position, so a user column that happens to be named `_row_id` is real data, not a
        ...
        let coalesce_row_id_leaf = phys_row_id_leaf.filter(|_| first_row_id.is_some());
        ...
        // (requested via `build_virtual_columns`/`_pos`, including whenever the projection
        // consists only of `PRUNABLE_METADATA_FIELDS`) or a physical metadata leaf unioned in
        // below. Pure-constant / `COUNT(*)` projections have neither and must keep reading all
        // columns to preserve the row count.
        //
        ...
        let metadata_only_projection = !project_field_ids.is_empty()
            && project_field_ids
                .iter()
                .all(|id| PRUNABLE_METADATA_FIELDS.contains(id));
        if project_field_ids_without_metadata.is_empty()
            && (project_pos
                || coalesce_last_updated_seq_leaf.is_some()
                || coalesce_row_id_leaf.is_some()
        ...
        if project_row_id {
            ...
            if first_row_id.is_some() && row_id_present_by_name_only {
                return Err(Error::new(
                    ErrorKind::FeatureUnsupported,
        ...
        // Build the batch stream. When `_row_id` is projected, synthesize it over the raw
        // parquet batches (using the reader-produced `_pos` position) before the transformer,
        // which then passes it through as a virtual field.
        let record_batch_stream = builder.build()?.map(move |batch| {
