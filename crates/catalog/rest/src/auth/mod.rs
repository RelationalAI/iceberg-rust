...
pub(crate) use custom::CustomAuthenticatorManager;
use iceberg::Result;
...
pub trait AuthSession: Debug + Send + Sync {
    ...
    /// Discards any cached authentication state (e.g. a cached token), so the next
    /// [`AuthSession::authenticate`] call re-derives it from scratch.
    ///
    /// Reachable through [`crate::RestCatalog::invalidate_token`]. Default no-op:
    /// sessions with nothing to cache (or that don't yet support forced invalidation)
